...
### `await bulid_market_id_symbol_map() -> dict`
**说明**：拉取全量订单簿并更新内部缓存。  
...
### `await create_market_order_with_tp_sl(symbol, side, quantity, take_profit_price, stop_loss_price, order_expiry=-1) -> (virtual_id, tuple)`
**说明**：市价单 + TP/SL，返回虚拟单号。
...
### `await calulate_worst_acceptable_price(symbol, side) -> float`
**说明**：计算市价单最差可接受价格（用于滑点保护）。
